...
class SELServices extends CommonHelperServices {
  ...
  async viewSELModule(req, res) {
    // ── NAYA PARAMETER ADDED: country ──
    const { month, year, country } = req.body
    ...
    let yrNum;
    let isGenericYear = false;

    // Agar year me "Year " string hai (e.g., "Year 1", "Year 2")
    if (typeof year === 'string' && year.toLowerCase().startsWith('year ')) {
      yrNum = parseInt(year.split(' ')[1], 10);
      isGenericYear = true;
    } else {
      // Purana format (e.g., "2025")
      yrNum = Number(year);
    }
    ...
    const formatYear = (num) => isGenericYear ? `Year ${num}` : num;

    // ── NAYA LOGIC: country field query me add kiya ──
    const pairs = [
      { country, month: MONTHS[prevIdx], year: prevIdx === 11 ? formatYear(yrNum - 1) : formatYear(yrNum) }, // prev month
      { country, month: MONTHS[idx], year: formatYear(yrNum) }, // current month
      { country, month: MONTHS[nextIdx], year: nextIdx === 0 ? formatYear(yrNum + 1) : formatYear(yrNum) }, // next month
    ]
  ...
  async generateAndSendPresignedUrls(req, res) {
    ...
    // ── NAYA PARAMETER ADDED: country ──
    const { filePaths, year, month, country } = req.body

    // Create promises for each file
    const presignedPromises = filePaths.map(async (url) => {
      const [folderName, fileName] = url.split('/')
      // ── NAYA LOGIC: country ko S3 path me add kiya ──
      const path = `${globalConstants.selModulePath}/${country}/${year}/${month.toLowerCase()}/${folderName}/`
  ...
  async addUpdateSelModule(req, res) {
    ...
    for (const filePath of filePaths) {
      ...
      const file = {
        fileName,
        // ── NAYA LOGIC: country ko S3 path DB me save karne ke liye add kiya ──
        path: `/${globalConstants.selModulePath}/${country}/${year}/${month.toLowerCase()}/${filePath}`,
    ...
    // ── NAYA LOGIC: country ko query me add kiya ──
    const selModule = await SELModule.findOne({ country, year, month })
    ...
    await SELModule.create({
      country, // ── NAYA LOGIC: create karte waqt country add kiya ──
      month,
      year,
      categories,
    })
