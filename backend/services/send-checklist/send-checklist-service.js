...
class SendChecklistServices extends SendChecklistHelperService {
    ...
    async fetchSendChecklistRecords(req, res) {
     ...
     async addSendChecklist(req, res) {
        ...
        // Check if required parameters are present
        if (!checklistForm) {
            return res.status(400).json(new FailureResponse(globalConstants.messages.fieldReuired.replaceField(ALL_FIELDS.CHECKLIST_FORM)))
        }
        ...
        if (
            ...
            return res.status(400).json(new FailureResponse(globalConstants.messages.invalidField(ALL_FIELDS.CHECKLIST_FORM)))
        }

        if (!studentData['user_id']) {
            return res.status(400).json(new FailureResponse(globalConstants.messages.invalidField.replaceField(ALL_FIELDS.STUDENT)))
        }
        ...
        finalCategoriesData.forEach((cat, index) => {
            ...
            if (cat.subCategories && cat.subCategories.length > 0) {
                ...
                cat.subCategories.forEach((sub) => {
                    const subQs = sub.Questions || sub.questions || [];
                    sub.score = utils.calculateScore(subQs);
                    catTotal += sub.score;
                });
                cat.score = catTotal;
            } else {
                const qs = cat.Questions || cat.questions || [];
                cat.score = utils.calculateScore(qs);
            }
        });
    ...
    async updateSendChecklist(req, res) {
        ...
        const { category, Questions, subCategories } = req.body
        ...
        if (!categoryToUpdate) {
            ...
            if (categoryToUpdate?.subCategories.length > 0) {
                ...
                categoryToUpdate.subCategories.forEach((subCategoryToUpdate, index) => {
                    const updatedSubCategory = subCategories[index]
                    if (updatedSubCategory) {
                        // BACKEND: Update subcategory details directly via calculation
                        const calculatedSubCategoryScore = utils.calculateScore(updatedSubCategory.Questions)
                        
                        subCategoryToUpdate.Questions = updatedSubCategory.Questions
                        subCategoryToUpdate.score = calculatedSubCategoryScore
                        totalSubCategoryScore += calculatedSubCategoryScore
                    }
                })

                // BACKEND: Update total category score directly
                categoryToUpdate.score = totalSubCategoryScore
                
            } else {
                // BACKEND: Update single category score directly
                const calculatedScore = utils.calculateScore(Questions)
                
                categoryToUpdate.Questions = Questions
                categoryToUpdate.score = calculatedScore
            }
