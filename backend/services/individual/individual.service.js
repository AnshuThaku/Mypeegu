...
class IndividualService extends CommonHelperServices {
	...
	async fetchIndividualCaseList(req, res) {
		try {
			...
			let filter = queryCombinations
			// Additional restriction for SSE Counselor: only records explicitly visible
			if (req.user?.permissions?.includes(globalConstants.SSECounselor)) {
				filter = { $and: [queryCombinations, { visibleToSSE: true }] }
			}
	...
	async fetchIndividualCaseDetails(req, res) {
		const { id } = req.params
		req.body['id'] = id
		const { error, message, statusCode, record } = await this.validateStudentDataAndUser(
			req,
			IndividualRecord,
		...
		if (req.user?.permissions?.includes(globalConstants.SSECounselor) && record) {
			if (record.visibleToSSE !== true) {
				return res
					.status(403)
