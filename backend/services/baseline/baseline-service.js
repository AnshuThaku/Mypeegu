...
//           )
//         } catch (error) {
//           console.error('Download filter transformation error:', error)
...
//   // 🟢 NEW: Submit Self-Assessment Baseline Data
//   async submitBaseline(req, res) {
//     try {
//       const {
//         studentId, schoolId, gradeBand, maxScorePerQuestion,
//         overallPercentage, tierPlacement, domainScores, responses,
//         baselineCategory
//       } = req.body;

//       if (!studentId || !schoolId || !baselineCategory) {
//         return res.status(400).json(new FailureResponse("Missing required fields."));
//       }

//       const student = await Students.findById(studentId).lean();
//       if (!student) return res.status(404).json(new FailureResponse("Student not found."));

//       const curAcademicYear = await this.getCurrentAcademicYear();
//       const validateStudentInAY = this.validateStudentAndAcademicYearInJourney(student, curAcademicYear._id);

//       if (!validateStudentInAY) {
//         return res.status(400).json(new FailureResponse("Student not mapped to current academic year journey."));
//       }
...
//         baselineCategory: baselineCategory, // Yahan 'Baseline 1', 'Baseline 2', ya 'Baseline 3' aayega
//         isSelfAssessment: true
//       });

//       if (existingRecord) {
//         return res.status(400).json(new FailureResponse(`You have already submitted ${baselineCategory} for this academic year.`));
//       }
...
//         isSelfAssessment: true, // 👈 THE MAGIC FLAG
        
//         gradeBand, maxScorePerQuestion, overallPercentage,
//         tierPlacement, domainScores, responses
//       };

//       await BaselineRecord.create(newAssessmentData);

//       return res.status(201).json(new SuccessResponse("Baseline Self-Assessment submitted successfully!"));

//     } catch (error) {
//       console.error("SUBMIT BASELINE ERROR:", error);
//       return res.status(500).json(new FailureResponse("Failed to submit assessment. Internal Server Error."));
//     }
//   }
// }
...
class BaselineService extends CommonHelperServices {
  /**
   * Fetches Baseline records... (No change in fetch)
   */
  async fetchBaselineRecordsList(req, res) {
    try {
      const { error, page, PAGE_SIZE, downloadAndFilter, skip, emptyData, academicYears } = await this.validateAndGetAYsAndPaginationData(req)
      if (error) return res.status(200).json(emptyData)

      const filterBody = req.body.filter || {}
      if (!utils.isAValidArray(filterBody.classroomIds) && req.user.permissions[0] === globalConstants.teacher) {
        const teacher = await Teacher.findOne({ email: req.user.email, isDeleted: {$ne: true} })
        filterBody.classroomIds = teacher.classroomsJourney.filter((obj) => obj.isAssigned).map((obj) => obj.classRoomId.toString())
      }
      ...
      const queryCombinations = this.fetchQueryCombinations(filteredStudents)
      if (queryCombinations === null) {
        return res.json(downloadAndFilter ? {} : { data: [], page, pageSize: PAGE_SIZE, totalCount: 0 })
      }

      let sortFields = [...globalConstants.baselineRecordSortFields, ...globalConstants.baselineRecordBasicSortFields]
      const sortOptions = utils.buildSortOptions(req.body, sortFields)
      ...
      if (downloadAndFilter) {
        try {
          transformedRecords = transformedRecords.map((item) => utils.baselineDataFormation(item, true))
        } catch (error) {
          return res.status(500).json({ error: 'Internal Server Error' })
        }
      }
      return res.json(downloadAndFilter ? transformedRecords : { data: transformedRecords, page, pageSize: PAGE_SIZE, totalCount })
    } catch (err) {
      return res.status(500).json({ error: 'Internal Server Error' })
    }
  }

  // 🟢 Create Record (With Trend Analysis)
  async createBaselineRecord(req, res) {
    const body = req.body
    const { error, message, school, SAY, academicYear } = await this.validateUserSchoolAndAY(req)
    if (error) return res.status(400).json(new FailureResponse(message))
    ...
    const student = await Students.findOne({ status: globalConstants.schoolStatus.Active, user_id: studentData.user_id, school }).lean()
    
    let studentErrMsg = null
    if (!student) studentErrMsg = globalConstants.messages.fieldNotFound.replaceField(ALL_FIELDS.STUDENT)
    else if (student.graduated) studentErrMsg = globalConstants.messages.alreadyGraduated
    else if (student.exited) studentErrMsg = globalConstants.messages.alreadyExited

    if (studentErrMsg) return res.status(404).json(new FailureResponse(studentErrMsg))

    const validateStudentInAY = this.validateStudentAndAcademicYearInJourney(student, academicYear._id)
    if (!validateStudentInAY) {
      return res.status(404).json(new FailureResponse(globalConstants.messages.fieldNotFoundInSelectedAY.replaceField(ALL_FIELDS.STUDENT)))
    }

    const baselineData = await BaselineRecord.findOne({
      studentId: student._id, classRoomId: validateStudentInAY.classRoomId, baselineCategory, SAY: SAY._id
    })

    if (baselineData) return res.status(400).json(new FailureResponse(globalConstants.messages.baseLineRecordAlreadyExist))
    ...
    for (const key of globalConstants.baselineRecordSortFields.filter((key) => key !== 'academicYear')) {
      if (!Array.isArray(studentData[key]?.data)) {
        return res.status(400).json(new FailureResponse(globalConstants.messages.invalidField.replaceField(key)))
      }
    }

    // 🟢 TREND ANALYSIS: Find the previous baseline record for this student
    const previousBaseline = await BaselineRecord.findOne({ 
        studentId: student._id, 
        academicYear: academicYear._id 
    }).sort({ createdAt: -1 });

    // 🟢 SCORING: Apply AI Scoring Engine and pass previous data
    studentData = baselineHelper.processBaselineScoring(studentData, previousBaseline)

    BaselineRecord.create(studentData)
      .then(() => res.status(201).json(new SuccessResponse(globalConstants.messages.baselineCreated)))
      .catch((error) => res.status(400).json(mongooseErrorHandler.handleError(error)))
  }

  // 🟢 Bulk Upload (With Trend Analysis)
  async uploadBaselineRecords(req, res) {
    const body = req.body.students || [{}]
    const { error, message, school, SAY, academicYear } = await this.validateUserSchoolAndAY(req)
    if (error) return res.status(400).json(new FailureResponse(message))

    const allStudents = await Students.find({ status: STATUSES.ACTIVE, school: school._id }, { user_id: 1, school: 1, studentName: 1, exited: 1, graduated: 1, classRoomId: 1, studentsJourney: 1 }).lean()
    
    let teacher = null
    const isTeacher = req.user.permissions[0] === globalConstants.teacher
    if (isTeacher) teacher = await Teacher.findOne({ email: req.user.email, isDeleted: {$ne: true} })

    const user_ids = body.map((b) => b.user_id)
    const baselineRecordsInDB = await BaselineRecord.find({ user_id: { $in: user_ids }, academicYear: academicYear._id }).select('user_id studentName school studentId baselineCategory classRoomId')
    ...
    for (let i = 0; i < body.length; i++) {
      let errors = false
      let studentData = body[i]
      const studentId = studentData['user_id']

      const studentInDB = allStudents.find((obj) => obj.user_id === studentId)
      if (!studentInDB) { errors = true; validationErrors.push(`Invalid student id at row number ${i + 2}`); continue }
      else if (studentInDB.graduated) { errors = true; validationErrors.push(`Student at row number ${i + 2} graduated.`); continue }
      else if (studentInDB.exited) { errors = true; validationErrors.push(`Student at row number ${i + 2} exited.`); continue }

      const validateStudentInAY = this.validateStudentAndAcademicYearInJourney(studentInDB, academicYear._id)
      if (!validateStudentInAY) { errors = true; validationErrors.push(`Student with ID: ${studentId} not found in academic year ${academicYear.academicYear} at row number ${i + 2}.`); continue }

      if (!studentData?.baselineCategory || !BASELINE_CATEGORIES.includes(studentData.baselineCategory)) { errors = true; validationErrors.push(`Baseline Category not Provided or invalid at row number ${i + 2}`); continue }

      const studentid_category = `${studentId}-${studentData?.baselineCategory}`
      if (uniqUsers.has(studentid_category)) { errors = true; validationErrors.push(`Row number ${i + 2} has duplicate Student Id field for baseline category - ${studentData?.baselineCategory}`); continue } 
      else uniqUsers.add(studentid_category)

      if (isTeacher && teacher) {
        const teacherClassroomIds = utils.isAValidArray(teacher?.classroomsJourney) ? teacher.classroomsJourney.filter((obj) => obj.isAssigned && obj.academicYear.toString() === academicYear._id.toString()).map((obj) => obj.classRoomId.toString()) : []
        if(!teacherClassroomIds.includes(validateStudentInAY.classRoomId.toString())) { errors = true; validationErrors.push(`You are not assigned to the classroom of Student for user_id: ${studentData.user_id} at row number ${i + 2}.`) }
      }

      if (!utils.isAValidString(studentData['baselineForm'])) { errors = true; validationErrors.push(`Row number ${i + 2} has invalid Class Group field`) }

      const baselineRecotdExist = baselineRecordsInDB.find((obj) => obj.studentId.toString() === studentInDB._id.toString() && obj.baselineCategory === studentData.baselineCategory && obj.classRoomId.toString() === validateStudentInAY.classRoomId.toString())
      if (baselineRecotdExist) { errors = true; validationErrors.push(`Baseline Record exists for Student ID ${studentData.user_id} for ${studentData?.baselineCategory} at row number ${i + 2}`) }
      ...
      if (!errors) {
        ...
        // 🟢 TREND ANALYSIS: Find previous baseline for this specific student in loop
        const previousBaseline = await BaselineRecord.findOne({ 
            studentId: studentInDB._id, 
            academicYear: academicYear._id 
        }).sort({ createdAt: -1 });

        // 🟢 SCORING: Apply AI Scoring Engine
        studentData = baselineHelper.processBaselineScoring(studentData, previousBaseline)
        recordsToInsert.push(studentData)
      }
    }

    if (validationErrors.length > 0) return res.status(400).json({ message: globalConstants.messages.invalidFileCheckError, validationErrors: validationErrors, fileContainsError: true })
  ...
  // 🟢 Update Record (With Trend Analysis)
  async updateBaselineRecord(req, res) {
    const body = req.body || {}
    const { error, message, statusCode, record: baselineRecord } = await this.validateStudentDataAndUser(req, BaselineRecord, ALL_FIELDS.BASELINE)
    if (error) return res.status(statusCode).json(message)

    const testData = (data, total, category) => {
      if (!Array.isArray(data)) return res.status(400).json(new FailureResponse(`${globalConstants.messages.invalidData} data in ${category} category`))
      if (!total) return res.status(400).json(new FailureResponse(`${globalConstants.messages.unexpectedData} total in ${category} category`))
      return true
    }

    // Update payload with new data
    let updatePayload = { ...baselineRecord._doc } // Mongoose doc se object liya

    globalConstants.baselineRecordSortFields.forEach((key) => {
      if (body[key]) {
        if (testData(body[key]?.data, body[key]?.total, key)) {
          updatePayload[key] = body[key]
        }
      }
    })

    // 🟢 TREND ANALYSIS: Find the record JUST BEFORE this current one
    const previousBaseline = await BaselineRecord.findOne({ 
        studentId: baselineRecord.studentId, 
        academicYear: baselineRecord.academicYear,
        _id: { $ne: baselineRecord._id } // Khudko chhodkar
    }).sort({ createdAt: -1 });

    // 🟢 SCORING: Apply AI Scoring Engine
    updatePayload = baselineHelper.processBaselineScoring(updatePayload, previousBaseline)

    // Set updated values back to Mongoose document
    Object.keys(updatePayload).forEach(key => {
        baselineRecord[key] = updatePayload[key]
    })
  ...
  // Submit Self-Assessment Baseline Data
  async submitBaseline(req, res) {
      try {
        const { studentId, schoolId, gradeBand, maxScorePerQuestion, overallPercentage, tierPlacement, domainScores, responses, baselineCategory } = req.body;
        if (!studentId || !schoolId || !baselineCategory) return res.status(400).json(new FailureResponse("Missing required fields."));
        ...
        const curAcademicYear = await this.getCurrentAcademicYear();
        const validateStudentInAY = this.validateStudentAndAcademicYearInJourney(student, curAcademicYear._id);
        if (!validateStudentInAY) return res.status(400).json(new FailureResponse("Student not mapped to current academic year journey."));
  
        const existingRecord = await BaselineRecord.findOne({ studentId: student._id, academicYear: curAcademicYear._id, baselineCategory: baselineCategory, isSelfAssessment: true });
        if (existingRecord) return res.status(400).json(new FailureResponse(`You have already submitted ${baselineCategory} for this academic year.`));
  
        const newAssessmentData = { studentId: student._id, user_id: student.user_id, studentName: student.studentName, school: schoolId, classRoomId: validateStudentInAY.classRoomId, academicYear: curAcademicYear._id, baselineCategory: baselineCategory, isSelfAssessment: true, gradeBand, maxScorePerQuestion, overallPercentage, tierPlacement, domainScores, responses };
        await BaselineRecord.create(newAssessmentData);
        return res.status(201).json(new SuccessResponse("Baseline Self-Assessment submitted successfully!"));
      } catch (error) {
        return res.status(500).json(new FailureResponse("Failed to submit assessment. Internal Server Error."));
