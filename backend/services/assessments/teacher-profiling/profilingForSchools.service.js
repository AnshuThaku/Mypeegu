...
async addSchoolProfiling(req, res) {
    try {
      ...
      // 🟢 1. Get current global academic year
      let curAcademicYear = await this.getCurrentAcademicYear()

      // 🟢 2. Try finding school's academic year based on global setting
      let SAY = await SchoolAcademicYears.findOne({
        school: new mongoose.Types.ObjectId(SchoolId),
        academicYear: curAcademicYear._id,
      }).lean()

      // 🟢 3. SMART FIX: Agar global year (e.g. 25-26) galat hai ya nahi mila, 
      // Toh jo date user ne select ki hai, uske according sahi Academic Year nikalo
      if (!SAY) {
        SAY = await SchoolAcademicYears.findOne({
          school: new mongoose.Types.ObjectId(SchoolId),
          startDate: { $lte: start_date.toDate() }, // Start date se pehle shuru hua ho
          endDate: { $gte: start_date.toDate() }    // Start date ke baad khatam ho raha ho
        }).lean()

        // Agar sahi date wala Academic Year mil gaya, toh current year ko usse replace kar do
        if (SAY) {
          curAcademicYear = { _id: SAY.academicYear }
        }
      }

      // 🟢 4. Agar ab bhi nahi mila, matlab sach mein school ka session configure nahi hai
      if (!SAY) {
        return res
          .status(400)
          .json(new FailureResponse("School Academic Year data not found for the selected dates. Please configure the academic year for this school first."));
      }
      ...
      console.error("ADD PROFILING ERROR ===>", error)
