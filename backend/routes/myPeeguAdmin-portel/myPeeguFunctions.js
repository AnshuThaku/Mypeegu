...
function fetchRequiredMyPeeguUsersPermissions(user) {
	...
	if (user.permissions) {
		const normalizedPermissions =
			user.permissions.includes(globalConstants.SSECounselor) &&
			!user.permissions.includes(globalConstants.ScCounselor)
				? [...user.permissions, globalConstants.ScCounselor]
				: user.permissions

		let userOperationsPermissions = {}
		normalizedPermissions.forEach((permission) => {
			userOperationsPermissions[permission] =
				globalConstants.permissions[permission]?.userOperationPermissions
		})
		let newPermissions = normalizedPermissions.flatMap((s) => {
			if (userOperationsPermissions.hasOwnProperty(s)) return userOperationsPermissions[s]
			return []
		})
...
function fetchAppFeatureList(permissions) {
	...
	let appFeatures = {}
	const normalizedPermissions =
		permissions.includes(globalConstants.SSECounselor) &&
		!permissions.includes(globalConstants.ScCounselor)
			? [...permissions, globalConstants.ScCounselor]
			: permissions

	normalizedPermissions.forEach((permission) => {
		appFeatures[permission] = globalConstants.permissions[permission]?.appFeatures
	})
	features = normalizedPermissions.flatMap((s) => {
		if (appFeatures.hasOwnProperty(s)) {
			return appFeatures[s]
		}
		features = []
