...
const CustomAutocompleteNew = ({
	...
	const getOptionLabel = (option) =>
		isObjectArray ? String(option?.label ?? '') : String(option ?? '')

	const isOptionEqualToValue = (option, val) => {
		if (isObjectArray) return option?.id === val?.id
		// for primitive options like year numbers, align on strict equality or stringified fallback
		return option === val || String(option) === String(val)
	}
