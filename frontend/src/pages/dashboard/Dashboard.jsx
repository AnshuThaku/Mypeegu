...
//   // 🟢 NAYA HELPER FUNCTION: Jo exact path match karega, aadha-adhura nahi
//   const checkIsActive = (pathToCheck) => {
//     if (!pathToCheck) return false;
//     const searchPath = pathToCheck.startsWith('/') ? pathToCheck : `/${pathToCheck}`;
//     return pathname.includes(searchPath);
//   };
...
//   // 🟢 YAHAN BHI SLASH (/) ADD KIYA HAI EXACT MATCH KE LIYE
//   useEffect(() => {
//     if (pathname.includes('/academic')) handleExpandCollapseMenus('academic')
//     if (pathname.includes('/initiations')) handleExpandCollapseMenus('initiations')
//     if (pathname.includes('/assessment')) handleExpandCollapseMenus('assessment')
//   }, [pathname])
...
//           routePath: routePaths.initiationsSelfAssessmentDashboard,
//           enable: isPermissionOfTeacher,
//         },
//       ],
//       enable: false,
...
//             const isParentActive = checkIsActive(sideBarItem.routePath); // 🟢 Helper function use kiya
//             return (
...
//                         minHeight: '44px',
//                         transition: 'all 0.2s',
//                         backgroundColor: isParentActive ? '#E3F2FD' : 'transparent',
//                         borderLeft: isParentActive ? '3px solid #1976D2' : '3px solid transparent',
//                         '&:hover': {
//                           backgroundColor: isParentActive ? '#E3F2FD' : '#F5F5F5',
//                         },
//                       }}
//                     >
//                       <ListItemIcon
//                         sx={{
//                           minWidth: drawerWidth === 300 ? '40px' : '0px',
//                           justifyContent: 'center',
//                         }}
//                       >
//                         <CustomIcon
...
//                               variant: 'body2',
//                               sx: { color: isParentActive ? '#1976D2' : '#424242' },
//                             }}
//                           />
//                           {sideBarItem?.nestedItems && (
//                             <CustomIcon
//                               name={
//                                 expandCollapseMenus[sideBarItem.routePath]
...
//                     </ListItemButton>
//                   </ListItem>
//                   {sideBarItem?.nestedItems && drawerWidth === 300 && (
//                       <Collapse in={expandCollapseMenus[sideBarItem.routePath]}>
//                         <List disablePadding sx={{ mt: 0.5 }}>
//                           {sideBarItem?.nestedItems?.map((nestedItem) => {
//                             const isChildActive = checkIsActive(nestedItem.routePath); // 🟢 Helper function use kiya
//                             return (
//                               !nestedItem?.enable && (
//                                 <ListItem key={nestedItem.id} disablePadding sx={{ mb: 0.5 }}>
//                                   <ListItemButton
//                                     onClick={() => {
//                                       navigate(nestedItem.routePath)
//                                     }}
//                                     sx={{
//                                       pl: 5,
//                                       py: 1,
...
//                                       '&:hover': {
//                                         backgroundColor: isChildActive ? '#E3F2FD' : '#F5F5F5',
//                                       },
//                                     }}
//                                   >
//                                     <Box
...
//                                         height: '6px',
//                                         borderRadius: '50%',
//                                         backgroundColor: isChildActive ? '#1976D2' : '#9E9E9E',
//                                         mr: 1.5,
//                                         transition: 'all 0.2s',
//                                       }}
//                                     />
//                                     <Typography
//                                       variant="caption"
//                                       sx={{ color: isChildActive ? '#1976D2' : '#616161' }}
//                                     >
//                                       {nestedItem.name}
//                                     </Typography>
//                                   </ListItemButton>
...
export default function Dashboard() {
  ...
  // 🟢 Removed 'new-assessment' from state, keeping only the main ones
  const [expandCollapseMenus, setExpandCollapseMenus] = useState({
    academic: false,
    initiations: false,
    assessment: false,
  ...
  // 🟢 NAYA HELPER FUNCTION: Updated to handle 'assessment' matching both routes
  const checkIsActive = (pathToCheck) => {
    if (!pathToCheck) return false;
    
    // Agar assessment parent hai, toh dono /assessment aur /new-assessment par active dikhaye
    if (pathToCheck === 'assessment') {
      return pathname.includes('/assessment') || pathname.includes('/new-assessment');
    }
  ...
  // 🟢 Update: Agar new-assessment ho tab bhi 'assessment' ka accordion khule
  useEffect(() => {
    if (pathname.includes('/academic')) handleExpandCollapseMenus('academic')
    if (pathname.includes('/initiations')) handleExpandCollapseMenus('initiations')
    if (pathname.includes('/assessment') || pathname.includes('/new-assessment')) handleExpandCollapseMenus('assessment')
  }, [pathname])
  ...
  const sideBarArray = [
    ...
    },
    {
      ...
      nestedItems: [
        ...
        // 🟢 NAYA ASSESSMENT ITEMS MERGED HERE
        {
          id: 101,
          name: localizationConstants.safeSpaces,
          routePath: routePaths.newAssessmentSafeSpaces,
          enable: false,
        },
        {
          id: 102,
          name: localizationConstants.selCompetency,
          routePath: routePaths.newAssessmentSELCompetency,
          enable: false,
        },
        {
          id: 103,
          name: localizationConstants.pulseDigitalWellbeing,
          routePath: routePaths.newAssessmentPulse,
          enable: false,
        },
        {
          id: 104,
          name: localizationConstants.heartEducators,
          routePath: routePaths.newAssessmentHeart,
          enable: false,
        },
      ],
      enable: false,
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      >
        ...
        >
          <List sx={{ px: 1.5, pt: 2, pb: 2 }}>
          {sideBarArray.map((sideBarItem) => {
            const isParentActive = checkIsActive(sideBarItem.routePath);
            return (
            <Box key={sideBarItem.id}>
              {!sideBarItem?.enable && (
                <Box sx={{ mb: 0.5 }}>
                  ...
                  {sideBarItem?.nestedItems && drawerWidth === 300 && (
                      <Collapse in={expandCollapseMenus[sideBarItem.routePath]}>
                        <List disablePadding sx={{ mt: 0.5 }}>
                          {sideBarItem?.nestedItems?.map((nestedItem) => {
                            const isChildActive = checkIsActive(nestedItem.routePath);
                            return (
                              !nestedItem?.enable && (
                                <ListItem key={nestedItem.id} disablePadding sx={{ mb: 0.5 }}>
                                  <ListItemButton
