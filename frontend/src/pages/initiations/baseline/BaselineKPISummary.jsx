...
const BaselineKPISummary = ({ analyticsData, schoolName }) => {
    ...
    // 3. 🟢 STRENGTH CALCULATION (Updated as per Doc)
    const overallStrengthTotal = analyticsData?.overallStrengthScore ?? 0;
    const avgStrength = studentsScreened > 0 
        ? Math.round(overallStrengthTotal / studentsScreened) 
        : 0;

    // 4. Strongest/Weakest Domains
    const domainData = analyticsData?.data ?? {}
    let strongestDomain = { name: '-', percentage: 0 }
    let weakestDomain = { name: '-', percentage: 100 }
    ...
    if (strongestDomain.percentage === 0) strongestDomain.name = '-'
    if (weakestDomain.percentage === 100 || weakestDomain.percentage === 0) weakestDomain.name = '-'
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* 🟢 NEW: Average Resilience Card */}
                <KPICard
                    title="Avg Resilience Buffer"
                    value={avgStrength}
                    subtitle={avgStrength >= 10 ? "🛡️ Strong Protective Buffer" : avgStrength >= 5 ? "⚡ Moderate Buffer" : "⚠️ Low Buffer"}
                    color={avgStrength >= 10 ? '#059669' : avgStrength >= 5 ? '#0EA5E9' : '#EF4444'}
                />
                ...
                <KPICard
                    title={localizationConstants.strongestDomain}
                    value={strongestDomain.name}
                    subtitle={strongestDomain.percentage > 0 ? `${strongestDomain.percentage}% Strength` : ''}
                    color={supportLevels.green.color}
                />
                ...
                <KPICard
                    title={localizationConstants.focusArea}
                    value={weakestDomain.name}
                    subtitle={weakestDomain.name !== '-' ? `${weakestDomain.percentage}% Proficiency` : ''}
                    color={supportLevels.orange.color}
                />
            </Box>
        </Box>
