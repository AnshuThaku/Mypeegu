...
import {
    ...
    InputAdornment,
    Button,
    Chip // 🟢 LOGIC UPDATE: Required for Tier UI if rendered directly here
} from '@mui/material'
...
const Baseline = () => {
    ...
    // 🟢 LOGIC UPDATE: selectedData structure updated to hold new fields
    ...
    // 🟢 LOGIC UPDATE: Now accepts all AI/Tier params correctly
    const handleSelectedData = useCallback((data, total, category, rowId, academicYear, aiInsights, overallTier, influenceSeverity) => {
        setSelectedData({ data, total, category, rowId, academicYear, aiInsights, overallTier, influenceSeverity })
  ...
  const renderScoreCell = (row, field, category) => {
   ...
   const renderCellContent = (column, row) => {
        ...
        // 🟢 FIX 1: Academic Year Visibility & Responsiveness Fix
        if (fieldName === 'academicYear') {
            let displayYear = '-';
            // Agar direct string format mein aaya hai (e.g. "2026-2027")
            if (typeof row?.academicYear === 'string' && row.academicYear.includes('-')) {
                displayYear = row.academicYear;
            } else {
                // Agar ObjectID hai toh array se find karo
                const yearObj = academicYears.find((ay) => ay._id === row?.academicYear);
                displayYear = yearObj?.academicYear || row?.academicYear || '-';
            }
            return (
               <Typography
    sx={{
        ...
        letterSpacing: '0.3px',
        flex: 1,
        whiteSpace: 'nowrap' // 🟢 YEH SABSE ZAROORI HAI: Isse text tootega nahi
    }}
>
    {column.label}
</Typography>
            ...
            return (
                <Typography
                    sx={{
                        fontSize: '13px',
                        color: '#334155',
                        whiteSpace: 'nowrap', // Responsive Fix
        ...
        if (fieldName === 'baselineCategory') {
            return <Typography sx={{ fontSize: '13px', color: '#334155', whiteSpace: 'nowrap' }}>{row?.baselineCategory || '-'}</Typography>
        }
        ...
        // 🟢 FIX 2 & 3: Tier Bracket Removal & Wrapping Fix
        if (fieldName === 'overallTier') {
            let tierStr = row?.overallTier || 'Tier 1';
            
            // Bracket ke andar ka text remove karna (e.g., "Tier 3 (Intensive)" -> "Tier 3")
            tierStr = tierStr.split('(')[0].trim();
            
            let bgColor = '#E5E7EB'; 
            let color = '#374151'; 

            if (tierStr.includes('Tier 3')) { bgColor = '#FEE2E2'; color = '#DC2626'; } 
            else if (tierStr.includes('Tier 2')) { bgColor = '#FEF3C7'; color = '#D97706'; } 
            else if (tierStr.includes('Tier 1')) { bgColor = '#DCFCE7'; color = '#059669'; } 
        ...
        // 🟢 FIX 3: Severity Wrapper Fix
        if (fieldName === 'influenceSeverity') {
            const severity = row?.influenceSeverity || 'None';
            let textColor = '#64748B'; 

            if (severity === 'Intervention recommended' || severity === 'Priority case' || severity === 'Priority') textColor = '#DC2626'; 
            else if (severity === 'Review') textColor = '#D97706'; 
            else if (severity.includes('Monitor')) textColor = '#2563EB'; 

            return (
                <Typography sx={{ fontSize: '12px', fontWeight: 600, color: textColor, whiteSpace: 'nowrap' }}>
                    {severity !== 'None' ? `⚠️ ${severity}` : '-'}
                </Typography>
            )
        }
    ...
    return (
        <Box sx={counsellorStyles.pageContainerSx}>
            ...
            <BaselineDrawer
                ...
                // 🟢 LOGIC UPDATE: Pass ALL new state properties to Drawer
                aiInsights={selectedData?.aiInsights} 
                overallTier={selectedData?.overallTier}
                influenceSeverity={selectedData?.influenceSeverity}

                isVersion2={(() => {
                    const yearStr = selectedData?.academicYear || "";
