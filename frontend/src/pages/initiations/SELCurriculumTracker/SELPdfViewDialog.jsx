...
// 🟢 FIX 1: Stable .mjs worker setup for pdf.js v4+
...
// 🟢 FIX 2: Optimized PDF configuration to prevent heavy auto-fetching
...
const SELPdfViewDialog = ({ open, onClose }) => {
 ...
 const handleFileClick = async (file, category) => {
    ...
    setSelectedPdfUrl(''); 
    try {
      // 🟢 FIX: File path se shuru ka slash hata do taaki double slash (//) na bane
      ...
      console.error("Secure URL fetch failed, using direct fallback silently");
      setSelectedPdfUrl(`${baseURL}${file.path}`);
    } finally {
  ...
  // 🟢 FIX 3: Precision Intersection Observer (No more jumping/early unmounting)
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(handler, {
      ...
      threshold: 0.3, // Focuses strictly on screen area
    })
            ...
            {isPdfFetching ? (
              ...
              >
                {Array.from(new Array(numPages || 0), (_, i) => {
                  ...
                  // 🟢 FIX 4: Goldilocks Memory Buffer (±5 pages for smooth scroll & low RAM usage)
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      position: 'relative', 
                      overflow: 'hidden',
                      // 🟢 FIX 5: Scroll layout shift protection (Strict solid dimensions)
