...
import {
    Grade_4_TableColumns_OLD,
    Grade_4_TableColumns_NEW,
    Grade_9_TableColumns_OLD,
    Grade_9_TableColumns_NEW,
    addOptions,
    checklistOptions,
...
const SendCheckList = () => {
	...
	return (
		<Box sx={counsellorStyles.pageContainerSx}>
			...
			{/* --- UPDATED LOGIC FOR CONDITIONAL COLUMNS --- */}
{(() => {
    // 1. Pehle pata karo ki filter mein kaunsa saal select hua hai
    const selectedYearObj = academicYears?.find(ay => ay._id === filterData?.selectdAYs?.[0]);
    
    // 2. Check karo ki kya wo 2026-2027 ya usse aage ka hai
    const isNewFormatTable = selectedYearObj?.academicYear >= "2026-2027";

    // 3. Checklist type aur Year ke hisaab se sahi columns choose karo
    const tableColumnsData = selectChecklist === checklistOptions?.[0]
        ? (isNewFormatTable ? Grade_4_TableColumns_NEW : Grade_4_TableColumns_OLD)
        : (isNewFormatTable ? Grade_9_TableColumns_NEW : Grade_9_TableColumns_OLD);

    return (
        <SendChecklistTable
            key={selectChecklist + (selectedYearObj?.academicYear || '')} // Key change takki table refresh ho
            ...
            columnsData={tableColumnsData} // <-- Updated Variable
