...
export const handleOptionChange = (
    ...
    if (!isGrade_9) {
        // --- GRADE 4 LOGIC ---
        // SAFE CHECK: Agar category ka data nahi hai ya array nahi hai, toh [] use karo
        const existingData = Array.isArray(Grade_4_Marks[mainTitle]) 
            ? Grade_4_Marks[mainTitle] 
            : [];
        
        const newScores = [...existingData];
        ...
        // --- GRADE 9 LOGIC ---
        if (!subQuestions) {
            // SAFE CHECK for simple categories
            const existingData = Array.isArray(Grade_9_Marks[mainTitle]) 
                ? Grade_9_Marks[mainTitle] 
                : [];
            ...
            dispatch(setGrade_9_Marks({ ...Grade_9_Marks, [mainTitle]: newScores }));
        } else {
            // SAFE CHECK for nested categories (Cognitive/Social)
            let categoryObject = (Grade_9_Marks[mainTitle] && typeof Grade_9_Marks[mainTitle] === 'object')
                ? { ...Grade_9_Marks[mainTitle] }
                : {};
            
            const existingSubData = Array.isArray(categoryObject[subTitle]) 
                ? categoryObject[subTitle] 
                : [];
            
            const newScores = [...existingSubData];
            ...
            categoryObject[subTitle] = newScores;
            dispatch(setGrade_9_Marks({ ...Grade_9_Marks, [mainTitle]: categoryObject }));
        }
    }
};
...
export const updateChecklist = async (
    ...
    if (questionListData?.subQuestions?.length > 0) {
        const subData = questionListData?.subQuestions?.map((data) => ({
            subCategory: categoriesNames[data?.localKey ?? ''],
            Questions: data?.selection ?? [],
            // FRONTEND SE SCORE HATA DIYA: Backend calculate karega
        }))
        body = {
            ...body,
            category: categoriesNames[questionListData?.localKey ?? ''],
