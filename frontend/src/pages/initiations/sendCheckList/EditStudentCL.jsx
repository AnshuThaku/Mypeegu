...
const EditStudentCL = ({
    ...
    studentId,
    onEditChecklist,
    isNewFormat = false, // <--- NAYA PROP: Table se pata chalega saal kaunsa hai
}) => {
    const flexStyles = useCommonStyles()
    const [isEditBtnClicked, setIsEditBtnClicked] = useState(false)
    const dispatch = useDispatch()
    ...
    }, [oldData, isEditBtnClicked])

    // Saal ke hisaab se options decide karo
    const optionsToDisplay = isNewFormat 
        ? ['No', 'Sometimes', 'Often'] 
        : ['Yes', 'No'];
            ...
            <Box sx={{ flexGrow: 1 }}>
                ...
                >
                    <QuestionsCollapsibleComponent
                        ...
                        isEditable={isEditBtnClicked}
                        isCollapsable={false}
                        optionsArray={optionsToDisplay} // <--- DYNAMIC OPTIONS PASS KIYE
                        qusetionChange={(
                            index,
                            option,
                            mainTitle,
