...
const SendChecklistTable = ({
    ...
    const prepareTableData = useMemo(() => {
        ...
        return checkListData.data.map((item) => {
            ...
            item?.categories?.forEach((cat) => {
                flatData[cat.category] = cat.score;
            });
            return flatData;
        });
    }, [checkListData?.data]);
    ...
    const renderScoreCell = (score, maxScore, onClickHandler) => {
        return (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                <Box
                    sx={{
    ...
    // =======================================================
    // 2. DYNAMIC CELL RENDERER
    // =======================================================
    const renderCellContent = (column, row, index) => {
        const fieldId = column.id;

        // Static Text Columns
        if (fieldId === localizationConstants.id) return row?.user_id ?? '-';
        if (fieldId === localizationConstants.academicYear) return row?.academicYear ?? localizationConstants.notApplicable;
        if (fieldId === localizationConstants.studentsName) return row?.studentName ?? '-';
        if (fieldId === 'createdAt') return formatDate(row?.createdAt) || '-';

        // Action Column
        if (fieldId === localizationConstants.showCategoryActions) return null; // Rendered directly in JSX

        // Dynamic Score Columns
        if (column.align === localizationConstants.center) {
            // Find category from row
            const categoryData = row?.categories?.find((d) => d?.category === column.id || d?.category === column.name);
            const score = categoryData?.score ?? 0;

            return renderScoreCell(score, 10, () => { // 10 is dummy maxScore for UI color gradient logic
                if (isSelectedAllForDelete) {
                    dispatch(setSCIdsForDelete(row?._id))
                } else {
                    // Determine format based on record's year
                    const isNewFormat = row?.academicYear >= "2026-2027";
                    
                    setEditTitle(column.id)
                    setStudentId(row?._id ?? '')
                    setDrawerOpen(true)
                    
                    // Setup Data in Redux based on Grade and Format
                    if (isGrade_9) {
                        dispatch(setGrade_9_Marks({
                            ...Grade_9_Marks,
                            [column.id]: categoryData?.Questions ?? [],
                        }))
                    } else {
                        dispatch(setGrade_4_Marks({
                            ...Grade_4_Marks,
    ...
    return (
        <Box sx={{ flex: 1, minWidth: 0, minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            {checkListData?.data?.length > 0 ? (
                <Box sx={{ ...tableStyles.container, flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0 }}>
                    ...
                    >
                        ...
                        >
                            ...
                            <TableBody>
                                {prepareTableData.map((row, index) => ( // <--- Renders using flattened data
                                    <TableRow
                                        key={row._id || index}
                                        onMouseEnter={() => setHoveredRowIndex(index)}
                                        onMouseLeave={() => setHoveredRowIndex(null)}
                    ...
                    <Box sx={tableStyles.footer}>
                        {!isSelectedAllForDelete ? (
                            <Box
                                ...
                                onClick={() => {
                                    const body = fetchAllSendChecklist(
                                        dispatch, filterData, searchText, currentPage, rowsPerPage.value, selectChecklist, sortKeys, true,
                                    )
                                    handleDownloadExcel(body, true)()
                                }}
                            >
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Button
                                    variant='outlined'
                                    sx={{
                                        height: '34px', borderColor: '#6A6A6A', color: '#6A6A6A',
                                        '&:hover': { borderColor: '#555', backgroundColor: 'rgba(106, 106, 106, 0.1)' },
                                    }}
                                    onClick={() => {
                                        dispatch(clearSCIdsForDelete([]))
                                ...
                                <Button
                                    variant='contained'
                                    sx={{ height: '34px', backgroundColor: '#DD2A2B', '&:hover': { backgroundColor: '#C62828' } }}
                                    onClick={() => setDeleteBulkDialog(true)}
                                >
                                    {localizationConstants.delete}
                                </Button>
                ...
                <Box sx={tableStyles.emptyState}>
                    <CustomIcon name={iconConstants.initiationBlack} style={{ width: '80px', height: '80px', opacity: 0.4 }} svgStyle={'width: 80px; height: 80px'} />
                    <Typography sx={tableStyles.emptyStateTitle}>{localizationConstants.noStudentChecklistMsg}</Typography>
                </Box>
            )}
          ...
          <EditStudentCL
                open={drawerOpen}
                setOpen={setDrawerOpen}
                // Determine `isNewFormat` dynamically for the Edit modal using the active row's Academic Year
                questionListData={(() => {
                    const rowData = checkListData?.data?.find(d => d._id === editStudentId);
                    const isNewFormat = rowData?.academicYear >= "2026-2027";
                    return isGrade_9 
                        ? Grade_9_Questions(Grade_9_Marks, isNewFormat)?.[editTitle ?? '']
                        : Grade_4_Questions(Grade_4_Marks, isNewFormat)?.[editTitle ?? ''];
                })()}
                isGrade_9={isGrade_9}
                oldData={isGrade_9 ? Grade_9_Marks : Grade_4_Marks}
                studentId={editStudentId ?? ''}
                ...
                // --- NAYI LINE YAHAN HAI ---
                isNewFormat={checkListData?.data?.find(d => d._id === editStudentId)?.academicYear >= "2026-2027"}
            />
            <CustomDialog
                isOpen={deleteStudentDialog}
                title={localizationConstants.deleteStudentRecord}
                ...
                onLeftButtonClick={() => setDeleteStudentDialog(false)}
                onRightButtonClick={async () => {
                    const res = await dispatch(checklistsingleDelete({ body: { id: singleDeleteId } }))
                    if (!res.error) {
                        setSingleDeleteId('')
                        setDeleteStudentDialog(false)
                        onEditChecklist()
