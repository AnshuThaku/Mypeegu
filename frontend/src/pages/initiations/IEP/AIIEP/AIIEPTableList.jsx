...
const AIIEPTableList = ({
    ...
    const getSortIcon = (column) => {
        ...
        return activeSortKey.value === sortEnum.asc 
    ...
    return (
        <Box sx={{ flex: 1, minWidth: 0, minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <Box sx={{ ...tableStyles.container, flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0 }}>
                                    ...
                                    {columns.map((column) => (
                                        ...
                                        >
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                ...
                                                {column.sort && <IconButton size='small' sx={{ p: 0 }}>{getSortIcon(column)}</IconButton>}
                                            </Box>
                                        </TableCell>
                                    ))}
                                </TableRow>
                    ...
                    <Box sx={tableStyles.footer}>
                        <Box
                            sx={tableStyles.downloadLink}
                            onClick={() => {
                                const body = fetchAllStudentIEP(dispatch, filterData, searchText, currentPage, rowsPerPage.value, sortKeys, true)
                                handleDownloadExcelForStudentIEP(body)()
                            }}
                        >
                            <DownloadIcon sx={{ fontSize: 16 }} />
                            <Typography sx={{ fontSize: '13px', fontWeight: 500 }}>{localizationConstants.downloadReport}</Typography>
                        </Box>
                        <CustomPagination rowsPerPage={rowsPerPage} setRowsPerPage={setRowsPerPage} currentPage={currentPage} setCurrentPage={setCurrentPage} totalCount={allStudentsForspecificSchool?.totalCount} />
                    </Box>
                </Box>
            ) : (
                <Box sx={tableStyles.emptyState}>
                    <CustomIcon name={iconConstants.initiationBlack} style={{ width: '80px', height: '80px', opacity: 0.4 }} />
                    <Typography sx={tableStyles.emptyStateTitle}>{localizationConstants.noStudentIEPMsg}</Typography>
                </Box>
            )}
            ...
            <CustomDialog
                ...
                message={localizationConstants.deleteStudentIEPMsg}
                onLeftButtonClick={() => setDeleteDialog(false)}
                onRightButtonClick={() => handleIepRecordDeletion(rowDataSelected._id, dispatch, setDeleteDialog, refreshListAndCloseDialog)}
                leftButtonText={localizationConstants.cancel}
                rightButtonText={localizationConstants.yesDelete}
            />
