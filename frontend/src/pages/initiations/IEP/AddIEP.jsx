...
import { useDispatch, useSelector } from 'react-redux'
import { Box, Typography } from '@mui/material' // 🟢 Typography import kiya message ke liye
import useCommonStyles from '../../../components/styles'
...
const AddIEP = forwardRef(
    (
        ...
        // 🟢 NAYA LOGIC: Selected Academic Year Pata Karna (2025 vs 2026)
        const selectedAYObj = academicYears.find(ay => ay._id === barFilterData.selectdAYs);
        const selectedAYString = selectedAYObj?.academicYear || "";
        const startYearMatch = selectedAYString.match(/(\d{4})/);
        const startYear = startYearMatch ? parseInt(startYearMatch[1], 10) : 0;
        
        // Agar saal 2026 ya usse zyada hai, toh AI Mode ON ho jayega
        ...
        // 🟢 SMART SAVE LOGIC: Agar AI mode hai, toh form validation ki zarurat nahi, sirf student select hona chahiye
        const disableSave = isAIEPMode 
        ...
        return (
            <Box className={flexStyles.flexColumn} gap={'20px'}>
                ...
                {student && Object.keys(student).length ? (
                    // 🟢 TRAFFIC POLICE LOGIC: AI Mode vs Legacy Mode
                    isAIEPMode ? (
                        <Box sx={{ mt: 3, p: 4, textAlign: 'center', bgcolor: '#F0F9FF', borderRadius: '12px', border: '2px dashed #3B82F6' }}>
                            <Typography variant="h6" color="#1E3A8A" fontWeight="700">
                                AI IEP Initialized 🚀
                            </Typography>
                            <Typography variant="body1" color="#334155" sx={{ mt: 1.5, maxWidth: '600px', mx: 'auto' }}>
                                For the 2026 academic year, manual entry is no longer required. 
                                Click <b>Save</b> to add this student to the IEP list. You can then generate their complete IEP automatically using AI on the dashboard!
                            </Typography>
                        </Box>
                    ) : (
                        <IEPForm
                ...
                {/* Data Loss & Cancel Dialogs are here as before... */}
                {/* Omitted for brevity but keep them EXACTLY as they were in your code */}
                {/* ... */}
